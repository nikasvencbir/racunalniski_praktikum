...
\newenvironment{definicija}{\begin{definition}}{\end{definition}}
...
\( p_n := \sum_{r=1}^{L} a_r \p{_r(p_{n-1})} \),
kjer je $L$ največja velikost skupine, $a_r$ delež skupin velikosti $r$, $\p{_r}$ pa verjetnost, 
da skupina s $r$ agenti podpira izbiro $A$.

Verjetnost $\p{_r}$ je izražena kot:
% začetek enačbe
\begin{align}
   \p{_{r}(p_{n-1})} =
   \begin{cases}
